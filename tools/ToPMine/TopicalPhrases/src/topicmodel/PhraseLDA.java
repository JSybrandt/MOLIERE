...
public class PhraseLDA {
	...
	public PhraseLDA(String path, int usrK, double usrAlpha, 
...
//		this.docLengthHistogram = new int[maxGroupNum+1];		
//		for(int i=0; i< docGroupNum.length; i++){
//			docLengthHistogram[docGroupNum[i]] ++;
//		}
		//this.topicDocHistogram = new int[this.K][maxGroupNum+1];	
		if(testFile != null){
	...
	public void inference(int iterations, boolean optFlag){
		...
		//this is to keep the account of word assignment
		int[] tmpDocTopicWordAssign =new int[this.K];
		...
		//at this iteration, we need to collect information for hyperParameter optimization 
			boolean optimizeFlag = optFlag && ((iter+1) % optInterval == 0) && (iter >= optBurnin);
			...
			for (int docInd = 0; docInd < this.docNum; docInd++){//index for documents
				...
				for (int groupInd = 0; groupInd < this.docGroupNum[docInd]; groupInd++){//index for group
					...
					for(int topicInd= 0; topicInd< this.K; topicInd++) {
						this.samplingProbabilities[topicInd] = (alpha[topicInd] + oneDocTopicCount[topicInd]); //first term
				...
				if( optimizeFlag ){ 
					for(int topicInd = 0; topicInd < K; topicInd++){
						...
						topicDocHistogram[topicInd][oneDocTrainingTopicCount[topicInd]]++;
					}
				}
			}// for each document
...
//		}
		alphaSum = Dirichlet.learnParameters(alpha,topicDocHistogram,docLengthHistogram);
		//alpha = alphaSum / K;
