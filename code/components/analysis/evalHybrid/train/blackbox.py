...
def search(f, box, n, m, batch, resfile,
    ...
    sample = np.array(np.transpose(lhsmdu.sample(d, batch)))

    # initial sampling
    for i in range(n//batch):
        points[batch*i:batch*(i+1),0:-1] = sample
        with executor() as e:
            points[batch*i:batch*(i+1), -1] = list(e.map(f, list(map(cubetobox, sample))))
        sample = np.array(np.transpose(lhsmdu.resample()))
        print(i*batch, '/', n, (i*batch/n)*100)
    ...
    # radius = 0.4
    # radius_step = radius / (batch - 1)
    # count = 0
    # for i in range(m//batch):
        # print(i * batch, "/", m)
        # best_point = points[points[:, -1].argsort()][0, :]
        # print("Best point:", best_point)
        # best_point = best_point[:, 0:-1]
        # small_box = [(p-radius, p+radius) for p in best_point]
        # small_box = [(max(box[i][0], l), min(box[i][1], h))
                     # for i, (l, h) in enumerate(small_box)]
        # radius -= radius_step
        ...
        # points[n+batch*i:n+batch*(i+1), 0:-1] = np.array(np.transpose(lhsmdu.resample()))
